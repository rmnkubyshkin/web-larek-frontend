 ...
 *  ``openCard`` - открыть карточку Продукта,
 * ``openBin`` - открыть корзину пользователя,
 * ``closePopup`` - закрыть модальное окно,
 * ``removeProduct`` - удалить продукт из корзины,
 * ``buyProduct`` - добавить продукт в корзину,
 * ``choosePayType`` - указать тип оплаты,
 * ``addAddressDelivery`` - указать адрес доставки,
 * ``addphoneNumber`` - указать телефон покупателя,
 * ``addEmail`` - указать электронны адрес покупателя, 
 * ``pay`` - оплатить заказ, 
 ...
 * ``selectPayType(): PayType`` - Указать тип оплаты, при оформлении заказа; 
